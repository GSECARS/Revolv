...
# ----------------------------------------------------------------------------------
# Project: Revolv
# File: revolv/model/pv_model.py
# ----------------------------------------------------------------------------------
# Purpose:
# This is the PV model of the Revolv application. It is responsible for defining
# process variables (PVs) and their interactions.
# ----------------------------------------------------------------------------------
...
class PVModel(ABC):
    ...
    rbv_extension: Optional[bool] = field(init=True, default=False, repr=True, compare=False)
        ...
        if self.limited:
            if with_limits:
                if value < caget(self.pv + ".LLM"):
                    ErrorMessageBox(msg=f"You reach the high limit of the {self.name}.")
                    return None
                elif value > caget(self.pv + ".HLM"):
                    ErrorMessageBox(msg=f"You reach the low limit of the {self.name}.")
                    return None
...
class StringValuePV(PVModel):
    ...
    def __post_init__(self) -> None:
        ...
        if self.monitor:
            object.__setattr__(self, "readback", caget(self._rbv_string, as_string=True))
            camonitor(self._rbv_string, callback=self._monitor_pv)
